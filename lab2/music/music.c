...
static int numSamples = 256;
static int numNotes = 8;
static double notes[] = {261.626, 293.665, 329.628, 349.228,
                            391.995, 440, 493.883, 523.251};
static unsigned int prescalars[8];
static unsigned int samples[256];
static int tone0 = 0, tone1 = 0;
static int music0_len = 30, music1_len = 7;
static int sample = 0;
...
// ode to joy
static unsigned int music0[] = {E4, E4, F4, G4, G4, F4, E4, D4, C4, C4, D4, E4, E4, D4, D4, 
                                E4, E4, F4, G4, G4, F4, E4, D4, C4, C4, D4, E4, D4, C4, C4};
                                // D4, D4, E4, C4, D4, E4, E4, F4, E4, C4, D4, E4, F4, E4, D4, C4, D4, G4,
                                // E4, E4, E4, F4, G4, G4, F4, E4, D4, C4, C4, D4, E4, D4, C4, C4};
// just the c scale                                
static unsigned int music1[] = {C4, D4, E4, F4, G4, A4, B4, C5};
...
void TIM2_IRQHandler(void)
{
    ...
    sample = (sample == numSamples - 1) ? 0 : (sample + 1);
    DAC->DHR12R1 = samples[sample];
}
...
void TIM4_IRQHandler(void)
{
    ...
    if (((GPIOC->IDR & (1 << 7))) == 0)
    {
        if (curr_music == 0)
        {
            speed0 = (speed0 == 4) ? 1 : (2*speed0);
            TIM4->ARR = 8000 / speed0;
        }
        else
        {
            speed1 = (speed1 == 4) ? 1 : (2*speed1);
            TIM4->ARR = 8000 / speed1;
        }
    ...
    unsigned int mtone = (curr_music == 0) ? music0[tone0] : music1[tone1];
    TIM2->PSC = prescalars[mtone];

    if (curr_music == 0)
        tone0 = (tone0 == music0_len - 1) ? 0 : (tone0 + 1);
    else
        tone1 = (tone1 == music1_len - 1) ? 0 : (tone1 + 1);
...
void computeSamples(void) {
    ...
    {
        tmp = (double)i*4.0*PI/numSamples;
        samples[i] = (unsigned int)(sin(tmp) + 1)*(0xFFF/2);
    }
    
    for (int i = 0; i < numNotes; i++)
        prescalars[i] = (unsigned int)(84000000.0/(double) numSamples/notes[i]) - 1;
}
...
int main(void)
{
    ...
    NVIC_SetPriority(EXTI0_IRQn, 4); // Priority level 4
