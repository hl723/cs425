...
void TIM4_IRQHandler(void)
{
    ...
    if (((GPIOC->IDR & (1 << 7))) == (0 << 7))
        tone = 1;

    GPIOC->MODER   |= (2 << 7*2);

    TIM2->PSC = prescalars[tone];
    tone = (tone == 7) ? 0 : (tone + 1);
}
...
int main(void)
{
    ...
    // NVIC_SetPriority(EXTI9_5_IRQn, 4); // Priority level 4 same as button
    ...
    // beep volume
    // uint8_t vol = 0x0F; // (0x1C - 12); // -6 - 12*2 dB
